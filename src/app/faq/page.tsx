        ...
        <div className="container mx-auto px-4">
            ...
            <div className="space-y-4">
              {faqs.map((faq, index) => (
                <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  >
                    <span className="font-semibold text-gray-900 pr-4">{index + 1}. {faq.question}</span>
                    <ChevronDown className={`h-5 w-5 text-[#177F00] flex-shrink-0 transition-transform duration-200 ${openIndex === index ? "rotate-180" : ""}`} />
                  </button>
                  <div className={`overflow-hidden transition-all duration-200 ${openIndex === index ? "max-h-96" : "max-h-0"}`}>
                    <div className="px-6 pb-4 pt-2 text-gray-600 leading-relaxed border-t border-gray-100">{faq.answer}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-16 max-w-3xl mx-auto">
            <div className="bg-gradient-to-r from-green-50 to-yellow-50 rounded-2xl p-8 border border-green-100">
              ...
              <p className="text-gray-600 text-center mb-6">Our support team is always ready to help you with any inquiries.</p>
              <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                <a href="mailto:info@kuleta.io" className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg hover:shadow-md transition-shadow">
                ...
                <a href="tel:+15023192096" className="flex flex-col items-center gap-2 p-4 bg-white rounded-lg hover:shadow-md transition-shadow">
                  <div className="p-3 bg-gradient-to-r from-[#177F00]/10 to-[#E99C00]/10 rounded-full">
