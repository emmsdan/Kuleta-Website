...
export default async function HomePage() {
  const [homeResult, categoriesResult, productsResult, siteConfig, announcement, hero, homeContent, story, founderItems, sponsors, partners, featuredIn, reviewItems, kuzaPage] = await Promise.all([
    getHomeCategoriesWithFallback(),
    getCategoriesWithFallback(),
    getProductsWithFallback(),
    ...
    getSingleton<{ heroImage?: string; videoUrl?: string; heroTitle?: string; heroSubtitle?: string }>("kuza.page", {}),
  ]);
  ...
  return (
    <HomePageContent
      ...
      hero={{
        ...
        backgroundImage: hero.backgroundImage || "/assets/b968a02aa95c98f64d11bb742ce5b76a609a84a9.png",
      }}
      homeContent={{
        ...
        reviewAuthor: reviewItems[0]?.title || homeContent.reviewAuthor || "Kuleta Customer",
      }}
      story={{
      ...
      kuza={{
        ...
        description: kuzaPage.heroSubtitle || "Building a new wave of women entrepreneurs across Africa",
        buttonLabel: "Learn More",
        imageUrl: kuzaPage.heroImage || "/assets/29d4542c8b108e0389991c23b1a793f7d500fb00.png",
        videoUrl: kuzaPage.videoUrl || undefined,
      }}
      sponsors={sponsors.map((item) => ({ title: item.title, imageUrl: item.imageUrl || undefined }))}
      partners={partners.map((item) => ({ title: item.title, imageUrl: item.imageUrl || undefined }))}
      featuredIn={featuredIn.map((item) => ({ title: item.title, imageUrl: item.imageUrl || undefined }))}
    />
  );
}
